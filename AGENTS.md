...
4. **Check network access** – ensure the environment can reach
   `https://registry.npmjs.org`, `https://cdn.playwright.dev`, and
   `http://archive.ubuntu.com`. The setup script downloads packages and
   browsers from these domains. If they are blocked, adjust your environment or
   proxy settings.
5. **Install dependencies** – run `npm run setup` at the repository root **before your first `npm run ci`**. Run it again whenever the container is restarted or if Playwright tests fail with messages like "Test was interrupted" or "page.evaluate: Test ended". This script unsets proxy variables, checks registry connectivity, runs `npm ci` in the root and `backend/`, and installs Playwright browsers.
   ...
   - Do not set `SKIP_PW_DEPS=1` until `CI=1 npx playwright install --with-deps` has completed successfully at least once. If `check-host-deps.js` reports missing libraries, rerun the setup without this variable to install them.
6. **Format code** – run `npm run format` in `backend/` to apply Prettier formatting.
7. **Run tests** – execute `npm test` in `backend/`. To run individual or root-level tests, use `node scripts/run-jest.js` instead of `npx jest` so dependencies resolve correctly. If tests cannot run because of environment limitations, mention this in the PR.
...
11. **Diagnose failures** – if setup or smoke tests fail unexpectedly, run `npm run diagnose` to launch a full pipeline check and Jest suite. The script prints `DIAGNOSTICS PASSED` on success and helps pinpoint environment issues.
