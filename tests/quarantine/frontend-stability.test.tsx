...
function loadComponent(rel) {
  ...
  src = src
    ...
    .replace(
      /from "https:\/\/esm\.sh\/@react-three\/fiber@8"/g,
      "from '@react-three/fiber'",
    )
    .replace(
      /from "https:\/\/esm\.sh\/@react-three\/drei@9"/g,
  ...
  const stubs = {
    "@react-three/fiber": {
      Canvas: (p) => React.createElement("canvas", p, p.children),
    },
...
describe("frontend stability", () => {
  ...
  test("ModelViewer snapshot", async () => {
    ...
    const { container } = render(
      React.createElement(ModelViewer, { url: "models/bag.glb" }),
    );
    expect(container.firstChild).toMatchSnapshot();
    const shot = await screenshotMarkup(container.innerHTML);
