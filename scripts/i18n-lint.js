...
async function main() {
  ...
  const templates = (await fs.readdir(templatesDir)).filter(f => f.endsWith('.txt'));
  ...
  for (const file of templates) {
    ...
    if (!translations[subjectKey]) missing.push(subjectKey); else unused.delete(subjectKey);
    if (!translations[bodyKey]) missing.push(bodyKey); else unused.delete(bodyKey);
  }
