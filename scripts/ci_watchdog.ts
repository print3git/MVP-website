...
async function handleCluster(slug: string, cluster: Cluster) {
  ...
  if (cluster.msg.includes("npm ci can only install packages")) {
    ...
    changed ||= await patchFiles(/npm ci --no-audit --no-fund/g,
...
async function globWalk(dir: string, ext: string, out: string[] = []): Promise<string[]> {
  const entries = await fs.readdir(dir, { withFileTypes: true });
  for (const e of entries) {
    const p = path.join(dir, e.name);
    if (e.isDirectory()) await globWalk(p, ext, out);
