...
async function handleCluster(slug, cluster) {
    ...
    if (cluster.msg.includes("npm ci can only install packages")) {
        ...
        changed ||= await patchFiles(/npm ci --no-audit --no-fund/g, "npm install --no-audit --no-fund");
    }
    ...
    if (cluster.msg.includes("Unauthorized: could not retrieve project")) {
        ...
        changed ||= await patchFiles(/netlify deploy [^\n]+/g, `if [[ -n "$NETLIFY_AUTH_TOKEN" && -n "$NETLIFY_SITE_ID" ]]; then
  ...
  echo "ℹ️ Skipping Netlify deploy – secrets missing"
    ...
    (0, node_child_process_1.execSync)('git config user.email "ci-watchdog@users.noreply.github.com"');
...
async function globWalk(dir, ext, out = []) {
    for (const e of await promises_1.default.readdir(dir, { withFileTypes: true })) {
