...
async function createRemoteOperator(name, email, trainingCompleted = false) {
  const { rows } = await query(
    "INSERT INTO remote_operators(name, email, training_completed) VALUES($1,$2,$3) RETURNING *",
    [name, email, trainingCompleted],
  );
  return rows[0];
}

async function listRemoteOperators() {
  const { rows } = await query("SELECT * FROM remote_operators ORDER BY id");
  return rows;
}

async function approveRemoteOperator(id, hubId = null) {
  const { rows } = await query(
    "UPDATE remote_operators SET approved=TRUE, hub_id=$2 WHERE id=$1 RETURNING *",
    [id, hubId],
  );
  return rows[0];
}
