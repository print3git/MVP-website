...
async function insertRemovalIncident(printerId, printJobId, error) {
  await query(
    "INSERT INTO removal_incidents(printer_id, print_job_id, error) VALUES($1,$2,$3)",
    [printerId, printJobId, error],
  );
}

async function getRecentRemovalIncidents(limit = 50) {
  const { rows } = await query(
    "SELECT * FROM removal_incidents ORDER BY id DESC LIMIT $1",
    [limit],
  );
  return rows;
}
