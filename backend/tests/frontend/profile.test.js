...
test('load displays models from API', async () => {
  const dom = new JSDOM('<div id="models"></div>', { runScripts: 'dangerously', url: 'http://localhost/profile.html' });
  ...
  dom.window.fetch = jest.fn(() => Promise.resolve({ json: () => [{ prompt: 'p', model_url: 'u', likes: 1 }] }));
