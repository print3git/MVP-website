export function validateCriticalEnv() {
  const { STRIPE_SECRET_KEY, FRONTEND_SUCCESS_URL, FRONTEND_CANCEL_URL } = process.env;
  if (!STRIPE_SECRET_KEY || !FRONTEND_SUCCESS_URL || !FRONTEND_CANCEL_URL) {
    const msg = "Missing STRIPE_SECRET_KEY, FRONTEND_SUCCESS_URL, or FRONTEND_CANCEL_URL";
