...
import { uploadFile } from "../lib/uploadS3";
...
router.get("/me", authRequired, async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error("me_fetch_failed", err as Error);
...
router.get("/profile", authRequired, async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error("profile_fetch_failed", err as Error);
    res.status(500).json({ error: "unexpected_error" });
  }
});
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error("avatar_upload_failed", err as Error);
      res.status(500).json({ error: "unexpected_error" });
    } finally {
      if (tempPath) {
        try {
...
router.post("/profile", authRequired, async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error("profile_upsert_failed", err as Error);
    res.status(500).json({ error: "unexpected_error" });
  }
});
