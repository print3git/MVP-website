...
app.post("/api/operators", async (req, res) => {
  const { name, email, trainingCompleted } = req.body || {};
  if (!name || !email) return res.status(400).json({ error: "Missing fields" });
  if (!isValidEmail(email))
    return res.status(400).json({ error: "Invalid email" });
  try {
    const op = await db.createRemoteOperator(
      name,
      email,
...
app.post("/api/admin/operators/:id/approve", adminCheck, async (req, res) => {
  const { hub_id } = req.body || {};
  try {
    const op = await db.approveRemoteOperator(req.params.id, hub_id || null);
    res.json(op);
  } catch (err) {
    logError(err);
