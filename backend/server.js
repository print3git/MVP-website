...
- List recent jobs with pagination
  ...
  try {
  const { rows } = await db.query(
  "SELECT _ FROM jobs ORDER BY created_at DESC LIMIT $1 OFFSET $2",
  [limit, offset],
  );
  res.json(rows);
  } catch (err) {
...
- Poll job status and retrieve the model URL when ready
  ...
  try {
  const { rows } = await db.query("SELECT _ FROM jobs WHERE job_id=$1", [
  req.params.jobId,
  ]);
  if (rows.length === 0) {
...
- Create a Stripe Checkout session
  ...
  try {
  const job = await db.query("SELECT job_id FROM jobs WHERE job_id=$1", [
  jobId,
  ]);
  ...
  return res.status(400).send(`Webhook Error: ${err.message}`);
      ...
      if (event.type === "checkout.session.completed") {
        ...
        try {
          ...
          if (!jobId) {
            const { rows } = await db.query(
              "SELECT job_id FROM orders WHERE session_id=$1",
              [sessionId],
            );
