...
async function getPrinterInfo(baseUrl, apiKey = "") {
  ...
  const printerRes = await axios.get(`${root}/api/printer`, {
    headers,
    timeout: 5000,
  });
  let status = "idle";
  ...
  let queueLength = 0;
  try {
    const jobRes = await axios.get(`${root}/api/job`, {
      headers,
      timeout: 5000,
    });
    if (
      jobRes.data &&
      jobRes.data.job &&
      jobRes.data.job.file &&
      jobRes.data.job.file.name
    ) {
      queueLength = 1;
    }
  } catch (_) {
...
async function restartPrinter(baseUrl, apiKey = "") {
  const sanitized = baseUrl.replace(/\/$/, "");
  await axios.post(
    `${sanitized}/api/system/commands/core/restart`,
    {},
    { headers: apiKey ? { "X-Api-Key": apiKey } : {} },
  );
}

async function shutdownPrinter(baseUrl, apiKey = "") {
  const sanitized = baseUrl.replace(/\/$/, "");
  await axios.post(
    `${sanitized}/api/system/commands/core/shutdown`,
    {},
    { headers: apiKey ? { "X-Api-Key": apiKey } : {} },
  );
}
